...
def process_zepto_task(self, file_paths):
    ...
    # Consolidate new Zepto products directly into product_master table
    try:
        from services.sync_to_product_master import sync_platform_to_master
        sync_platform_to_master('Zepto')
    except Exception as e:
        print(f"[ProductMasterSync] Error running master sync for Zepto: {e}")
